...
const MARKDOWN_WALK_BODY = `const bt = String.fromCharCode(96);
const root = sel
...
export async function markdown(pageId: string, selector?: string): Promise<string | undefined> {
    if (cdpManager) {
        try {
            const session = await cdpManager.ensureSession(pageId);
            return await session.evaluate(`const sel = ${JSON.stringify(selector ?? null)};\n${MARKDOWN_WALK_BODY}`);
        } catch (err) { logCdpFallback(err); }
    }
    return runPlaywrightCode(pageId, `return await page.evaluate((sel) => {\n${MARKDOWN_WALK_BODY}\n}, ${JSON.stringify(selector ?? null)});`);
}
...
export async function screenshotSlice(pageId: string, slice: number, width?: number, height?: number): Promise<McpContent[]> {
    if (cdpManager) {
        try {
            ...
            const normalizedSlice = normalizeSlice(rawSlice, totalSlices);
